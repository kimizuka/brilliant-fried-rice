...
void loop() {
  ...
  if (isOn) {
    ...
    if (-0.8F < accZ) {
      ...
      for (int i = 0; i < 16; i++) {
        pixels.setPixelColor((ledIndex + i * 2) % NUMPIXELS, pixels.Color(250, 120, 40));
      }
...
void handleButtonRelease() {
  ...
  if (isOn) {
    OscWiFi.send(ip, port, "/on", 1);
  } else {
    OscWiFi.send(ip, port, "/off", 0);
  }
}

void doOpen() {
  OscWiFi.send(ip, port, "/open", 1);
}

void doClose() {
  OscWiFi.send(ip, port, "/close", 0);
}